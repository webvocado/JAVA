...
public class Ex03_NumberOfCurrency {
	...
	public static void main(String[] args) {
		 ...
		 * 2. (입력금액) 입력
		 * 
		 * 3. 화폐매수 계산 [조건] : 큰 화폐단위부터 계산한다. 3-1. 화폐매수 계산 수식 10 = 537620 / 50000 (화폐매수) =
		 * (입력금액) / (화폐단위) 3-2. 잔액 계산 (잔액) = 537620 - 500000 = 37620 = (입력금액) -
		 * ((화폐단위)*(화폐매수)) = (입력금액) % (화폐단위) 3-3. 화폐단위 변환 번갈아 가면서, /5, /2 연산을 반복한다.
		 * (화폐단위) = (화폐단위) / 5 (화폐단위) = (화폐단위) / 2
		 * 
		 * 4. 3번의 과정을 반복한다. - 반복조건 : (화폐단위) 1보다 크거나 같으면 반복
		 * 
